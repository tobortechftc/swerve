...
public class TeleopH extends SwerveUtilLOP {
    ...
    public void runOpMode() throws InterruptedException{
        ...
        while (opModeIsActive()) {
            ...
            all_js = Math.abs(gamepad1.left_stick_x) + Math.abs(gamepad1.left_stick_y) +
                    Math.abs(gamepad1.right_stick_x) + Math.abs(gamepad1.right_stick_y);

            if (robot.use_swerve) {
                if (all_js > 0.2) { // decide chassis movements
                    if (robot.cur_mode == SwerveDriveHardware.CarMode.CAR) { //
                        ...
                        st_sh = 0.5;
                        snake_sh = 0.1;
                    } else { // robit or crab mode
                        turn_sh = 0.5;
                        st_sh = 0.5;
                        snake_sh = 0.5;
                    }
                    //if (gamepad1.left_trigger > 0.1) {
                    if (Math.abs(gamepad1.left_stick_y) > turn_sh &&
                            Math.abs(gamepad1.right_stick_y) > turn_sh &&
                            Math.abs(gamepad1.left_stick_y - gamepad1.right_stick_y) > turn_sh) {
                        // Swerve Turn
                    ...
                    } else if (Math.abs(gamepad1.left_stick_x) > crab_sh &&
                            Math.abs(gamepad1.right_stick_x) > crab_sh &&
                            Math.abs(gamepad1.left_stick_x + gamepad1.right_stick_x) > 2 * crab_sh) {
                    ...
                    } else if ((Math.abs(gamepad1.left_stick_y) > st_sh || Math.abs(gamepad1.right_stick_y) > st_sh)&&
                            (Math.abs(gamepad1.left_stick_y+gamepad1.right_stick_y)>st_sh)){
                        if (robot.cur_mode != SwerveDriveHardware.CarMode.CAR) {
